...
import { planningGlobalData } from '@/lib/pdf/data';
...
export async function GET() {
  ...
  const buf = await pdfPlanning(planningGlobalData(snap, struct));
  return new Response(new Uint8Array(buf), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': 'inline; filename="planning-escale.pdf"',
    },
  });
}
